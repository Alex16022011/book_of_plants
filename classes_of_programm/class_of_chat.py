...
class MenuOfChats(CTk):
    def create_new_chat(self):
        ...
        def go_to_menu_of_chats() -> None:
            ...
            btn3 = CTkButton(master=frm2, text='+', corner_radius=1000, bg_color='#33FF33', font=('Arial', 40), width=3, command=self.create_new_chat)
            btn3.place(x=width1-555, y=height1-555)

        def place_exit_button_when_create_a_chat() -> None:
            global btn3
            btn = CTkButton(master=root, text='←', font=('Arial', 20), width=100, height=47, command=go_to_menu_of_chats)
            btn.pack(anchor='nw')
